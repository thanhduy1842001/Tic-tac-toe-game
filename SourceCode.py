...
class Application(tk.Frame):
	...
	def draw_header_frame(self):
		...
		self.x_label = tk.Label(self.score_frame, text=f'X   {self.x_score}', fg='red',
						 font=('Papyrus', 18, 'bold'))
		self.x_label.grid(row=0, column=0, padx=20, pady=5)

		self.o_label = tk.Label(self.score_frame, text=f'O   {self.o_score}', fg='blue',
						 font=('Papyrus', 18, 'bold'))
		self.o_label.grid(row=0, column=1, padx=110, pady=5)

		current = self.players[self.current_player]
		self.player_label = tk.Label(self.player_frame, text=f'Current Player : {current}', fg='black',
						 font=('verdana', 12, 'bold'))
		self.player_label.grid(row=0, column=0, padx=60, pady=15)
	...
	def draw_body_frame(self):
		...
		self.canvas = tk.Canvas(self.body, width=300, height=300, bg='aquamarine')
		self.canvas.grid()
	...
	# Bắt sự kiện đánh quân cờ từ người dùng
	...
	def write_text(self, pos, text):
		if text == "X":
			self.canvas.create_text(pos,  text=text, font='Chiller 50 bold', fill="red")
		else:
			self.canvas.create_text(pos,  text=text, font='Chiller 50 bold', fill="blue")

	# Kiểm tra ô có còn trống không ?	
	...
	def check_winner(self, mark):
		...
		if ch[0]:
			...
			if mark == "X":
				self.canvas.create_line(pos, width=5, fill='red')
			else:
				self.canvas.create_line(pos, width=5, fill='blue')
			return True
		else:
			return False
	...
	# Hiện bảng thông báo mỗi khi ván cờ kết thúc
